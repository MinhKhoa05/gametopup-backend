...
### [2026-05-19]
**Topic: HttpOnly Cookie Refresh Token & Clean Web Architecture API**
- **Type**: Architecture Decision / Refactoring
- **Tags**: [auth] [security] [cookie] [refactor] [api-design]
- **Scope**: API Layer, BLL (Service Layer), Integration Tests
- **Impact**: Tối ưu hóa tính bảo mật của Refresh Token, giảm thiểu bề mặt tấn công XSS, làm sạch mã nguồn Controller bằng Cookie Extensions, và chuẩn hóa cấu hình môi trường test.

**Bài học rút ra**:
- **Security-First Auth (HttpOnly Cookie)**: Lưu trữ Refresh Token vào cookie loại `HttpOnly`, `SameSite=Lax` thay vì trả về trong response body. Điều này giúp ngăn chặn hoàn toàn việc script độc hại phía Client (XSS) tiếp cận và đánh cắp Refresh Token.
- **Clean API & DTO Isolation**: `AuthResponseDTO` chỉ nên trả về các trường dữ liệu thực sự cần thiết cho client (như `AccessToken` và thông tin `User`). Loại bỏ hoàn toàn `RefreshToken` khỏi payload JSON để tối ưu payload và giảm thiểu rủi ro rò rỉ.
- **Controller Refactoring via Cookie Extensions**: Đóng gói toàn bộ logic tương tác Cookie (`SetRefreshToken`, `GetRefreshToken`, `DeleteRefreshToken`) vào lớp static `CookieExtensions` mở rộng cho `HttpResponse`/`HttpRequest`. Điều này tuân thủ triệt để nguyên lý DRY và giúp mã nguồn Controller cực kỳ súc tích, chỉ tập trung vào điều hướng luồng.
- **Delete Options Consistency**: Khi xóa cookie bằng `response.Cookies.Delete(...)`, bắt buộc phải truyền kèm cùng một cấu hình `CookieOptions` (`HttpOnly`, `Secure`, `SameSite`) tương tự như lúc tạo. Việc không đồng bộ cấu hình này có thể khiến trình duyệt bỏ qua việc xóa cookie trên một số môi trường.
- **Test Environment Isolation (JWT Secret Key)**: Cấu hình cứng khóa bí mật kiểm thử (ví dụ: thông qua `builder.UseSetting` trong `CustomWebApplicationFactory`) giúp bộ integration tests hoạt động ổn định và độc lập khỏi các tệp `.env` hay `appsettings.json` cục bộ, tránh lỗi cryptographic khi tạo Access Token do khóa quá ngắn.

---

### [2026-05-19]
**Topic: Elegant Fluent Test Infrastructure, Minimalist Factories & Strict Domain Semantics**
- **Type**: Architecture Decision / Refactoring / Test Best Practice
- **Tags**: [testing] [fluent-api] [clean-code] [test-factory] [domain-semantics]
- **Scope**: Integration Tests
- **Impact**: Tối ưu hóa toàn diện trải nghiệm viết test, nâng cao tính trực quan của phần Arrange/Act/Assert, giảm 90% lượng code trùng lặp, đảm bảo tính bền vững trước thay đổi lược đồ cơ sở dữ liệu (schema-resilience), và chuẩn hóa nghiệp vụ miền.

**Bài học rút ra**:
- **Fluent Request Extensions (`WithTestAuth`, `WithJson`)**: Thay thế các helper cồng kềnh bằng các extension method gọn gàng trên `HttpRequestMessage` hỗ trợ chainable/fluent API. Thiết kế này giúp code phần Act trở nên mạch lạc, tách bạch rõ ràng giữa cấu hình Request và quá trình gửi nhận Client.
- **Minimalist Factory Seeds (Stable Signatures)**: Chỉ đưa các trường bắt buộc khóa ngoại (Foreign Keys) hoặc định danh duy nhất (Unique Keys) làm tham số bắt buộc trên phương thức seed (như `SeedGamePackageAsync` chỉ nhận `gameId` và `name`). Tất cả các thuộc tính tùy biến cấu hình khác đều được ủy thác qua duy nhất 1 lambda action `customize: x => { ... }`. Thiết kế này giúp bộ khung test hoàn toàn bền vững trước mọi thay đổi thuộc tính của DB (schema-resilience).
- **Generic Fluent Request/Response Extensions (`WithCookie`, `GetCookie`, `WithJson`, `WithTestAuth`)**: Tận dụng triệt để sức mạnh của Extension Methods trong C# để xây dựng một bộ công cụ hoàn toàn generic và tái sử dụng cho mọi kịch bản kiểm thử (không phân biệt Auth, Order hay Game). Thiết kế này vừa loại bỏ hoàn toàn mã nguồn thô của `HttpRequestMessage` ra khỏi các Scenario Tests, vừa ngăn chặn sự rò rỉ phụ thuộc DTO hay endpoint đặc thù của riêng module nào vào hạ tầng dùng chung.
- **Strict Domain Semantics (UnitPrice vs Total)**: Trong viết kịch bản test, luôn tuân thủ chặt chẽ ngữ nghĩa nghiệp vụ miền (Domain Semantics). Ví dụ: `UnitPrice` của đơn hàng phải phản ánh giá trị của 1 gói (`packagePrice`), còn tổng giá trị hoàn tiền phải được tính toán động hoặc gán rõ ràng bằng `packagePrice * orderQuantity` thay vì gán nhầm lẫn trực tiếp. Điều này giúp test case tự tài liệu hóa nghiệp vụ cực kỳ chính xác.
- **State-Verification for Inventory (Stock Assurance)**: Kịch bản kiểm thử quy trình quan trọng (như Hủy đơn hàng đồng thời - CancelOrder) bắt buộc phải tích hợp assert kiểm soát tồn kho (`StockQuantity`). Assert này giúp đảm bảo cơ chế hoàn tồn kho hoạt động nguyên tử và chính xác đúng 1 lần (idempotency check) dưới áp lực tải đồng thời.
- **Private Helper Consolidation**: Khi phát hiện lặp lại cấu trúc Arrange cơ bản trong hàng loạt test case (ví dụ: seed cùng 1 bộ Game, Package và Customer), việc rút gọn thành một hàm helper dùng chung (`SeedBaseDataAsync`) trả về kiểu dữ liệu **Tuple** giúp loại bỏ tối đa code boilerplate nhiễu, làm nổi bật logic kiểm thử cốt lõi.
- **Explicit Concrete Collection Attribute**: Tránh đặt `[Collection("IntegrationTests")]` trên lớp abstract cơ sở. Luôn khai báo nó tường minh trực tiếp trên từng lớp test cụ thể để xUnit nhận diện và điều phối chính xác luồng chạy tuần tự, ngăn ngừa tuyệt đối xung đột tài nguyên.
- **Local Private Test Helpers (Encapsulation of Domain Details)**: Để tránh lặp mã nguồn Arrange (như xây dựng `CreateUserRequest` hay `LoginRequest` nhiều lần) mà không làm rò rỉ hay ô nhiễm các DTO đặc thù nghiệp vụ vào lớp hạ tầng dùng chung (`HttpClientExtensions`), việc định nghĩa các hàm `private async Task` nội bộ ngay trong chính concrete Test Class (ví dụ: `RegisterAsync`, `LoginAsync`, `RegisterAndLoginAsync`) là giải pháp kiến trúc hoàn hảo nhất. Thiết kế này giúp các kịch bản test giữ được tính mạch lạc tối đa, che giấu các chi tiết khởi tạo DTO thô, trong khi vẫn tái sử dụng mượt mà `HttpClient` và các fluent extension dùng chung.
- **Direct Standard HttpClient Calls for Simple Requests**: Đối với các yêu cầu HTTP đơn giản không chứa header tùy biến, tham số bảo mật hay Cookie (ví dụ: các yêu cầu GET công khai như lấy danh sách Game, chi tiết gói), ta nên gọi trực tiếp các phương thức chuẩn của `HttpClient` (như `Client.GetAsync`, `Client.PostAsJsonAsync`) thay vì khởi tạo thủ công `new HttpRequestMessage` và gửi qua `SendAsync`. Thiết kế này giúp loại bỏ tối đa mã thừa nhiễu, tập trung hoàn toàn vào ngữ cảnh Act/Assert.
- **HttpClient Per-Test Isolation Lifecycle**: Thay vì chia sẻ chung 1 instance của `HttpClient` được khởi tạo trong constructor, việc khởi tạo trì hoãn và recreate lại Client mới ở mỗi kịch bản test (`InitializeAsync` chạy trước mỗi test case) giúp cô lập hoàn toàn trạng thái Cookie, Headers, Connection Pool giữa các test, đảm bảo độ sạch (cleanliness) tối đa và chống flaky test.
